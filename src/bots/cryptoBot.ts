...
//         step: "welcome",
//         botType: "crypto",
//         retryCount: 0, // Add retryCount to session
//       };
//     } else {
...
//     const sessionKey = `crypto:${user.telegramId}`;
...
//         session.step = "welcome"; // Reset step for retry
//         await redis.set(sessionKey, JSON.stringify(session), "EX", 86400); // 24h TTL

//         if (user.rejectionReason === "no_kyc") {
...
// import { sendAdminAlertCrypto } from "../utils/services/notifier-crypto";
// import { generateCaptcha, verifyCaptcha } from "../utils/captcha";
// import { isValidBybitUID} from "../utils/validate";
// import rateLimit from "telegraf-ratelimit";
// import { BotContext } from "../telegrafContext";
// import dotenv from "dotenv";
// import Redis from "ioredis";
...
// const BYBIT_LINK = process.env.BTBIT_LINK;
...
//         `✅ <b>Step 1:</b> Solve the Captcha 🔢\n` +
//         `✅ <b>Step 2:</b> Choose Your Country 🌍\n` +
//         `✅ <b>Step 3:</b> Register at Bybit / Weex and provide your UID \n` +
...
//     ctx.session.step = "weex_confirmed";
//     // Update Redis session
//     const sessionKey = `crypto:${ctx.from.id}`;
//     await redis.set(sessionKey, JSON.stringify(ctx.session), "EX", 86400);
...
//   bot.action("continue_to_weex", async (ctx) => {
//     if (!ctx.session || ctx.session.step !== "weex_confirmed") return;
...
//     const stepNumber = ctx.session.bybitUid || !ctx.session.hasBybitAccount ? 4 : 3;

//     await ctx.replyWithHTML(
//       `<b>🚀 Step ${stepNumber}: Weex Registration</b>\n\n` +
//         `<b>Why Weex?</b>\n` +
//         `🌎 <i>Global access – No KYC required!</i>\n\n` +
//         `📌 <b>Sign up here</b> 👉 <a href="${WEEX_LINK}">Weex Registration Link</a>\n\n` +
//         `✅ Click <b>Done</b> after completing your registration!`,
...
//       } else if (error.message.includes("UID must be provided")) {
//         errorMessage =
//           "🚫 No Weex UID provided. Please start over with /start.";
//       } else if (
//         error.name === "MongooseError" ||
//         error.name === "MongoServerError"
//       ) {
...
//         const details = ctx.session.bybitUid
...
//         throw new Error("Weex UID must be provided");
//       }
...
//         country: ctx.session.country,
//         status: "approved",
//         weexUid: ctx.session.weexUid,
//       };

//       if (ctx.session.bybitUid) {
//         updatePayload.bybitUid = ctx.session.bybitUid;
//         updatePayload.registeredVia = "both";
//       } else {
