...
import { message } from "telegraf/filters";
import { Afibe10XUserModel, IAfibe10X_User } from "../models/afibe10x_user.model";
...
const WEEX_LINK = process.env.WEEX_LINK || "https://www.weex.global/en/register?vipCode=afb10x";
...
export default function (bot: Telegraf<BotContext>) {
    ...
    bot.start(async (ctx) => {
        ...
        const user = await Afibe10XUserModel.findOne({ telegramId: tgId, botType: "afibe10x" });
        ...
        switch (ctx.session.step) {
            case "captcha": {
                if (verifyCaptcha(text, ctx.session.captcha || "")) {
                    ...
                    await redis.set(sessionKey, JSON.stringify(ctx.session), "EX", 86400);
                    await ctx.replyWithHTML(`❌ <b>Failed.</b> Please try again.\nType this number 👉 <code>${newCaptcha}</code>`);
                }
                break;
            }
            case "submit_uid": {
    ...
    bot.catch((err, ctx) => {
        console.error(`🚨 Afibe 10X Bot Error for update ${ctx.update.update_id}:`, err);
