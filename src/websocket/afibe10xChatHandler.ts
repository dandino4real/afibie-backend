...
export function setupAfibe10xWebSocket(
  ...
  console.log("🌐 Afibe10X WS listening on /afibe10x-chat");
  ...
  wss.on("connection", (ws, req) => {
    ...
    if (!adminId) {
      ws.close(1008, "adminId required");
      return;
    }
    ...
    ws.on("close", () => {
      const i = adminClients.findIndex((c) => c.ws === ws);
  ...
  /* ---------- REDIS → WS FANOUT ---------- */
  redisSub.subscribe("afibe10x:user_message");
  ...
  redisSub.on("message", (_, payload) => {
    ...
    console.log(`📡 Message delivered to ${delivered} admin(s)`);
  });
  ...
  globalThis.afibe10xChatHandler = {
    async sendToAdmin(telegramId: string, text: string) {
      ...
      // 🔥 publish instead of WS fanout
      await redisPub.publish(
        "afibe10x:user_message",
  ...
  console.log("✅ Afibe10X WS fully initialized");
}
